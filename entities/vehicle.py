...
class Vehicle(Tools):
    ...
    def __init__(self, vehicle_data, recorder: object, reader: object, updater: object) -> None:
        self.license_plate = f'{vehicle_data['license_plate']}' if vehicle_data['license_plate']  else None
        self.driver = f'{vehicle_data['driver']}' if vehicle_data['driver'] else None
        ...
        self.last_route = f'{vehicle_data['last_route']}' if vehicle_data['last_route'] else None
        self.service_type = f'{vehicle_data["service_type"]}' if vehicle_data["service_type"] else None

        if vehicle_data['service_type']:

            aggregateFleetChecker = 'Van agregada' in vehicle_data['service_type'] or 'Utilitário' in vehicle_data['service_type'] or 'Veículo de passeio' in vehicle_data['service_type']

            if not aggregateFleetChecker: self.license_plate_type = '"Frota fixa"'
            else:  self.license_plate_type = '"Frota agregada"'
        else: self.license_plate_type = '"Frota fixa"'
    ...
    def isVehicleExists(self):
        vehicle_data = self.reader.getSearchVehicleData(license_plate=self.license_plate)
    ...
    def updateVehicleDriver(self, driver: str):
        if driver is None or not driver in self.driver:
            self.updater.setUpdateVehicleStrData(field_to_insert='driver', data_to_insert=f'{self.driver}', field_to_check='license_plate', data_to_check=f'{self.license_plate}')  

    def updateVehicleAssistant(self, driver: str):
        if driver is None or not driver in self.driver:
            self.updater.setUpdateVehicleStrData(field_to_insert='assistant', data_to_insert=f'NULL', field_to_check='license_plate', data_to_check=f'{self.license_plate}')

    def updateVehicleServiceType(self, service_type: str):
        if service_type is None or not service_type in self.service_type:
            self.updater.setUpdateVehicleStrData(field_to_insert='service_type', data_to_insert=f'{self.service_type}', field_to_check='license_plate', data_to_check=f'{self.license_plate}')