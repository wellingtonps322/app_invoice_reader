...
class Payments(Tools):
    ...
    def getData(self, series_body, preinvoice_header):
        ...
        row_payment_dict['service_type'] = self.getCheckingServiceType(series_body)
        # ? ID route
        row_payment_dict['id_route'] = self.getIDRoute(row=series_body)
        # ? Service center checking
        row_payment_dict['service_center'] = self.getCheckingServiceCenter(series_body, route_number=row_payment_dict['id_route'])
        # ? KMs range
        row_payment_dict['kms_range'] = self.getKmsRange(series_body)
        # ? Special day
        row_payment_dict['special_day'] = self.getCheckingSpecialDay(series_body)
        # ? Part time route
        row_payment_dict['part_time_route'] = self.getCheckingPartTimeRoute(series_body)
        # ? Ambulance
        row_payment_dict['ambulance'] = self.getCheckingAmbulance(series_body)
        # ? Start date
        row_payment_dict['start_date'] = self.getDate(row=series_body, column='Data de início')
        # ? End date
        row_payment_dict['end_date'] = self.getDate(row=series_body, column='Data de término')
        # ? License plate
        if series_body['Placa']:
            row_payment_dict['license_plate'] = f'"{series_body['Placa']}"'
        # ? Driver
        if series_body['Motorista']:
            row_payment_dict['driver'] = f'"{series_body['Motorista'].lower()}"'
        # ? Driver ID
        if 'Visited addresses' in series_body['Descrição']:
            row_payment_dict['id_driver'] = self.getIdDriver(description=series_body['Descrição'])
        # ? Quantity
        if series_body['Quantidade']:
            row_payment_dict['quantity'] = int(series_body['Quantidade'])
        # ? Unit price
        if series_body['Preço unitário']:
            row_payment_dict['unit_price'] = float(series_body['Preço unitário'])
        # ? Total
